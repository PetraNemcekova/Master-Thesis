...
met_arr, gen_arr, gen_feature_names, identicifators = fcn.PrepareForPrediction(met_preprocessed, gen_preprocessed) 

# Multivariate 

RF = RandomForestRegressor()
met_pred = cross_val_predict(RF, gen_arr.values, met_arr.values, cv=5)
...
for i in range(met_arr.shape[1]):
    ...
    RF = RandomForestRegressor()
    met_pred = cross_val_predict(RF, gen_arr.values, met.reshape(-1,1), cv=5)
...
metabolites_results_df.to_csv('RF_crossval_univariate_metabolites_results.csv')
