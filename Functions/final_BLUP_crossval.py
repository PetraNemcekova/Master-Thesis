...
[max_mae, min_mae, mean_mae, sd_mae, max_mse, min_mse, mean_mse, sd_mse,
 max_cc, min_cc, mean_cc, sd_cc] = fcn.evaluateMultiModel(met_arr.values,
                                                          met_pred)
...
for i in range(met_arr.shape[1]):
    ...
    met_pred = cross_val_predict(
        myBLUP, gen_arr.values, met_arr.values[:, i], cv=5)
