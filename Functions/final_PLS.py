...
met_train, met_test, gen_train, gen_test = train_test_split(met_arr, gen_arr, test_size = 0.2,random_state=0)
...
for i in range(met_train.shape[1]):
    ...
    met_pred = myPLS.predict(gen_test.values) 
    [mae, mse, cc] = fcn.evaluationParametersSingleMetaboliteModel(met_test.values[:,i], met_pred[:,0])
