...
met_arr, gen_arr, gen1_feature_names, identificators = fcn.PrepareForPrediction(met_preprocessed, gen_preprocessed) 
...
for i in range(met_arr.shape[1]):
    ...
    myPLS = PLSRegression()    
    met_pred = cross_val_predict(myPLS, gen_arr.values, met.reshape(-1,1), cv=5)
    [mae, mse, cc] = fcn.evaluationParametersSingleMetaboliteModel(met_arr.values[:,i], met_pred[:,0])
...
metabolites_results_df.to_csv('sPLS_crossval_univariate_metabolites_results.csv')
